...
void ThroughputTracer()
{
    ...
    Simulator::Schedule(MilliSeconds(100), &ThroughputTracer);
}
...
int main(int argc, char *argv[])
{
    ...
    ApplicationContainer sinkApp = sinkHelper.Install(nodes.Get(9));
    sinkApp.Start(Seconds(0.0));
    sinkApp.Stop(Seconds(61.0));
    sink = DynamicCast<PacketSink>(sinkApp.Get(0));

    BulkSendHelper bulk("ns3::TcpSocketFactory",InetSocketAddress(interfaces[8].GetAddress(1), port));
    bulk.SetAttribute("MaxBytes", UintegerValue(0));  
    ApplicationContainer senderApp = bulk.Install(nodes.Get(1));
